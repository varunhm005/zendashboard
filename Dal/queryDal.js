const queryModel = require('../Models/queryModel');
const mongoose = require('mongoose');

async function createQuery(data) {
    try {

        const randomNumber = Math.floor(Math.random() * Math.pow(10, 5));
        data.queryId = "QN" + randomNumber.toString().padStart(5, '0')

        let payload = new queryModel(data)
        let result = await payload.save()
        if (result) {
            return { code: 200, status: true, message: "Query created successfully", data: result }
        }
        return { code: 202, status: false, message: "Error while creating mentor", data: {} }

    } catch (err) {
        return { code: 500, status: false, message: err.message }
    }
}

async function getAllQuery() {
    try {
        let result = await queryModel.find().lean()
        if (result) {
            return { code: 200, status: true, message: "Query fetched successfully", data: result }
        }
        return { code: 202, status: false, message: "Error while creating mentor", data: {} }

    } catch (err) {
        return { code: 500, status: false, message: err.message }
    }
}


module.exports = { createQuery, getAllQuery };